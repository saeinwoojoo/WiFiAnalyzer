...
class AccessPoint {
    ...
    static final int NO_RSSI_SIGNAAL = -100;

    static final int NUMBER_OF_LEVELS = 101; // The number of levels to consider in the calculated level.
    ...
    public int compareTo(AccessPoint other) {
        ...
        if (networkId != INVALID_NETWORK_ID /*WifiConfiguration.INVALID_NETWORK_ID*/
                && other.networkId == INVALID_NETWORK_ID /*WifiConfiguration.INVALID_NETWORK_ID*/) return -1;
        if (networkId == INVALID_NETWORK_ID /*WifiConfiguration.INVALID_NETWORK_ID*/
                && other.networkId != INVALID_NETWORK_ID /*WifiConfiguration.INVALID_NETWORK_ID*/) return 1;
    ...
    public static int convertFrequencyToChannel(int frequency) {
        if (frequency >= 2412 && frequency <= 2484) {
            return (frequency - 2412) / 5 + 1; // Channel in 2.4GHz
        } else if (frequency >= 5170 && frequency <= 5825) {
            return (frequency - 5170) / 5 + 34; // Channel in 5GHz
        } else {
            return -1;
        }
    }

    public static String getGHzFromFrequency(int frequency) {
        if (frequency >= 2412 && frequency <= 2484) {
            return "2.4 GHz";
        } else if (frequency >= 5170 && frequency <= 5825) {
            return "5.0 GHz";
     ...
     *         (both inclusive).
     */
    public static int getLevel(int rssi, int numLevels) {
